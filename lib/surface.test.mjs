...
import { SITE, INDEXABLE, DISALLOWED, MCP_PACKAGE, mcpInstallLine } from "./site.mjs";
...
test("the welcome page never prints an install command that cannot work", () => {
  /*
   * THE DEFECT THIS PINS. /welcome handed a paying customer
   * `npx -y github:Yuve21/proof-team-mcp`, and that repository does not exist,
   * so the command fails. Telling somebody who has just paid to run something
   * that cannot work is the same class as the apply form posting to example.com:
   * it looks finished and it is not.
   */
  /*
   * COMMENTS STRIPPED FIRST, and this is the third time in one session a
   * "must not contain X" check has fired on the comment explaining why X is
   * forbidden. The pattern is general enough to be worth stating: a test
   * forbidding a string will always fire first on the file that documents the
   * ban, because that file is the one place the string legitimately appears.
   * Strip comments and assert against code, or the check ends up weakened.
   */
  const codeOnly = (src) => src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/(^|[^:])\/\/.*$/gm, "$1");
  const welcome = codeOnly(read("../app/welcome/page.tsx"));

  // No hard-coded package anywhere in the page's CODE.
  assert.ok(
    !welcome.includes("proof-team-mcp"),
    "the package must come from lib/site.mjs, not be typed into the page",
  );
  assert.match(welcome, /mcpInstallLine/, "the page must ask for the line rather than build one");

  // And when there is no package, there is no line to print.
  if (!MCP_PACKAGE) {
    assert.equal(mcpInstallLine("proof1.k1.a.b"), null, "with no package published there is no honest line");
    assert.match(welcome, /is being\s+published and is not ready yet/, "and the page must say so instead");
  } else {
    const line = mcpInstallLine("proof1.k1.a.b");
    assert.match(line, /npx -y /);
    assert.ok(line.includes(MCP_PACKAGE));
  }
});
